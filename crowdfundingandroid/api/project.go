...
func ProjectCreate(c *gin.Context) {
	...
	if err := c.ShouldBind(&projectUpload); err == nil {
		res := projectUpload.Create(claim.Id)
		c.JSON(200, res)
	} else {
		c.JSON(400, err)
	}
}
func ProjectUploadFile(c *gin.Context) {
	...
	claim, _ := utils.ParseToken(c.GetHeader("Authorization"))
	if err := c.ShouldBind(&projectUpload); err == nil {
		res := projectUpload.UploadFile(claim.Id, file, fileSize, c.Param("pid"))
		c.JSON(200, res)
	} else {
		c.JSON(400, err)
	}
}
func ProjectUploadInfo(c *gin.Context) {
	var projectUpload service.ProjectService
	claim, _ := utils.ParseToken(c.GetHeader("Authorization"))
	if err := c.ShouldBind(&projectUpload); err == nil {
		res := projectUpload.UploadInfo(claim.Id, c.Param("pid"))
		c.JSON(200, res)
	} else {
		c.JSON(400, err)
	}
